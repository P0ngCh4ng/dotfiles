...
3. Done! Cage sandbox now allows access to the new project

**Project-Specific Rules**:
- **Global rules**: `~/.claude/rules/common/project-management.md` - High-level project management strategy
- **Project details**: come straight from `projects.yml` (single source of truth). Each session working inside a registered project is told its ports/DB/always_on/start_command automatically by the `announce-managed-project.py` SessionStart hook; `pj-info <name>` shows the same on demand.
...
- **Global rules** (apply to ALL projects):
  - `~/.claude/rules/common/project-management.md` - High-level project management strategy
  - `~/.claude/rules/dotfiles/database-management.md` - Database operations and workflows
- **Implementation documentation**: `~/.claude/skills/db-management/SKILL.md`
- **Per-project details** live only in `projects.yml`; the `announce-managed-project.py` SessionStart hook injects them into each session.
